  ...
  <div class="example-layout">
    ...
    <section class="example-main">
      <div class="example-header">
        <h2 class="example-title">Date Picker</h2>
        <p class="example-desc">
          Single-selection calendar with min/max date range. Shows all months between bounds.
        </p>
      </div>
    ...
    <ControlPanel>
      ...
      <div class="control-group">
        ...
        <div class="radio-group">
          <label class="radio-label">
            <input type="radio" v-model.number="firstDayOfWeek" :value="0" />
            Sunday
          </label>
          <label class="radio-label">
            <input type="radio" v-model.number="firstDayOfWeek" :value="1" />
            Monday
          </label>
        </div>
      </div>
      ...
      <div class="control-group">
        ...
        <select v-model="localeKey" class="control-select">
          <option v-for="opt in LOCALE_OPTIONS" :key="opt.key" :value="opt.key">
            {{ opt.label }}
          </option>
        </select>
      </div>
      ...
      <div class="control-group">
        ...
        <label class="toggle-label">
          <input type="checkbox" v-model="fullWeeks" class="control-checkbox" />
...
// ── Remount key: changes whenever any option changes ─────────────────
const reinitKey = computed(() =>
  [firstDayOfWeek.value, localeKey.value, minDateStr.value, maxDateStr.value, fullWeeks.value, disabledDateStrs.value.join(',')].join('|')
);
...
/* Control styles (shared across pages via global or scoped) */
.control-group {
  display: flex;
  flex-direction: column;
  gap: 8px;
